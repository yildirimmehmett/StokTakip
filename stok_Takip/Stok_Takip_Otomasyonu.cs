...
{
    public partial class Stok_Takip_Otomasyonu : Form
    {
        public Stok_Takip_Otomasyonu()
        {
            InitializeComponent();
        }
        bool durum;
        ...
        {
            if (txtbarkodno.Text != "" && txtürünadı.Text != "" && txtsatışfiyatı.Text != "" && txtmiktar.Text != "" && txttoplamfiyat.Text != "")
            {
                kontrol();
                if (durum == true)
                {
                    bağlanti.Open();
                    SqlCommand komut = new SqlCommand("insert into sepeteekle(tc,adsoyad,telefon,barkodno,urunadi,miktarı,satışfiyatı,toplamfiyat,tarih) values(@tc,@adsoyad,@telefon,@barkodno,@urunadi,@miktarı,@satışfiyatı,@toplamfiyat,@tarih)", bağlanti);
                ...
                {
                    bağlanti.Open();
                    SqlCommand komutmu = new SqlCommand("update sepeteekle set miktarı = miktarı+ '" + int.Parse(txtmiktar.Text) + "' where tc='" + txttc.Text + "'", bağlanti);
                    komutmu.ExecuteNonQuery();
                    SqlCommand komut = new SqlCommand("update sepeteekle set toplamfiyat=miktarı*satışfiyatı where tc='" + txttc.Text + "'", bağlanti);
                    komut.ExecuteNonQuery();
                    bağlanti.Close();
                }
        ...
        private void btnSatışSil_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dataGridView1.CurrentRow.Cells[i].Value.ToString() != "")
                {
                    bağlanti.Open();
                ...
                {
                    MessageBox.Show("Tablo Boş Silinecek'te Bişey Yok", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
        ...
        private void btnsatışyap_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < dataGridView1.Rows.Count - 1; i++)
            {
                ...
                komut.Parameters.AddWithValue("@tarih", DateTime.Now.ToString());
                komut.ExecuteNonQuery();
                SqlCommand komut2 = new SqlCommand("update uruntablosu set miktarı=miktarı-'" + int.Parse(dataGridView1.Rows[i].Cells[5].Value.ToString()) + "' where barkodno='" + dataGridView1.Rows[i].Cells[3].Value.ToString() + "' where tc='" + dataGridView1.Rows[i].Cells[0].Value.ToString() + "'", bağlanti);
        ...
        private void btnsatışiptal_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                {
                    MessageBox.Show("Tablonuz Zaten Boş Neyi Silecen Daha", "BİLGİ", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
        ...
        private void txtbarkodno_TextChanged(object sender, EventArgs e)
        {
            ...
            bağlanti.Open();
            SqlCommand komut = new SqlCommand("select *from uruntablosu where barkodno like '" + txtbarkodno.Text + "'", bağlanti);
            SqlDataReader oku = komut.ExecuteReader();
            while (oku.Read())
            {
        ...
        private void txttc_TextChanged(object sender, EventArgs e)
        {
            ...
            bağlanti.Open();
            SqlCommand komut = new SqlCommand("select *from müşteri where tc like '" + txttc.Text + "'", bağlanti);
            SqlDataReader oku = komut.ExecuteReader();
            while (oku.Read())
            {
        ...
        private void şİFREGÜNCELLEToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        {
           DialogResult secim =  MessageBox.Show("Çıkış Yapmak İstedğinizden Emin Misiniz ?", "Çıkış Yapılıyor", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void gÜNCELLEVEYASİLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            güvenkik_sorusu_güncelle guvenlıksılşguncelle = new güvenkik_sorusu_güncelle();
            guvenlıksılşguncelle.ShowDialog();
        }
    }
}
